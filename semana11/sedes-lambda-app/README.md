...
```bash
sedes-lambda-app$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
sedes-lambda-app$ sam local start-api
sedes-lambda-app$ curl http://localhost:3000/
```
...
```bash
sedes-lambda-app$ sam logs -n NetCodeWebAPIServerless --stack-name sedes-lambda-app --tail
```
...
```bash
sedes-lambda-app$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name sedes-lambda-app
```
